...
class HexType(Interpolation):
    ...
    def pre_calc(self, mesh):
        ...
        fcenter = np.array([np.sum(vpt[:,:,0].reshape(((self.order+1)**2, Neles, Nvar),order = 'F'),axis=0)/(self.order+1)**2,
                           np.sum(vpt[:,:,-1].reshape(((self.order+1)**2, Neles, Nvar),order = 'F'),axis=0)/(self.order+1)**2,
                           np.sum(vpt[:,0,:].reshape(((self.order+1)**2, Neles, Nvar),order = 'F'),axis=0)/(self.order+1)**2,
                           np.sum(vpt[:,-1,:].reshape(((self.order+1)**2, Neles, Nvar),order = 'F'),axis=0)/(self.order+1)**2,
                           np.sum(vpt[0].reshape(((self.order+1)**2, Neles, Nvar),order = 'F'),axis=0)/(self.order+1)**2,
                           np.sum(vpt[-1].reshape(((self.order+1)**2, Neles, Nvar),order = 'F'),axis=0)/(self.order+1)**2])
