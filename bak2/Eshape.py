...
class InterpolationShape(object):
    ...
    def __init__(self, order):
        ...
        self._fmap = {
            ...
            'hex': {'quad': [[0, order, (order+1)**2 - 1, (order+1)*order],
                             [(order+1)*order, (order+1)**2 - 1, -1, -1 - order],
                             [-1 - order, -1, -(order + 1)**2 + order, -(order + 1)**2],
                             [-(order + 1)**2, -(order + 1)**2 + order, order, 0],
                             [0, (order+1)*order, -1 - order, -(order + 1)**2],
                             [-(order + 1)**2 + order, -1, (order+1)**2 - 1, order]]},
...
class HexType(InterpolationShape):
    ...
    def nsps(self, poly_order):
        return (poly_order+1)**3


    def pre_calc(self, mesh):
        # corner points
        cmesh = np.array([mesh[self._fmap['hex']['quad'][i]] for i in range(6)])   # it has shape [Nedges, Npt in each edge, Nele, Nvars]
        # Face center
        fcenter = np.sum(cmesh,axis=1) / 4
        # element plane normal
        fnormal = np.cross(cmesh[:,2] - cmesh[:,0] , cmesh[:,1] - cmesh[:,3])
        ...
        for k in range(self.order+1):
            for l in range(self.order+1):
                for j in range(self.order+1):
                    temp.T[m] = msh.T[0]**k * msh.T[1]**l * msh.T[2]**j
                    m += 1
